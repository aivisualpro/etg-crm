#!/usr/bin/env node
/**
 * Migrate "lang" table from AppSheet → BigQuery as "etgLanguage"
 *
 * Usage:  node scripts/migrate-appsheet-language.mjs
 */
...
console.log('📡 Fetching data from AppSheet...')
...
await dataset.createTable(BQ_TABLE, { schema: { fields: schema }, location: 'US' })
...
for (let i = 0; i < bqRows.length; i += BATCH_SIZE) {
    ...
    try {
        ...
        if (err.name === 'PartialFailureError') {
            ...
            console.log(`   ⚠️  Batch ${batchNum}/${totalBatches}: ${batch.length - failCount} inserted, ${failCount} failed`)
...
console.log(`\n🎉 Migration complete! ${bqRows.length} rows migrated to ${BQ_PROJECT_ID}.${BQ_DATASET}.${BQ_TABLE}\n`)
