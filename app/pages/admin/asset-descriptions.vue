...
async function fetchData() {
  ...
  try {
    const data = await $fetch<{
      ...
      descriptions: any[]
      total: number
    }>('/api/bigquery/asset-descriptions')
    if (data.success) {
...
function toggleSort(col: string) {
  if (col === 'image_url') return // not sortable
...
const filteredRows = computed(() => {
  ...
  if (search.value) {
    ...
    rows = rows.filter((r: any) =>
      (r.eng && r.eng.toLowerCase().includes(q))
      || (r.arabic && r.arabic.toLowerCase().includes(q))
      || (r.A67 && r.A67.toLowerCase().includes(q))
      || (r.A66_label && r.A66_label.toLowerCase().includes(q)),
    )
  }
  return rows
})
  ...
  <div class="w-full flex-1 min-h-0 flex flex-col">
    ...
    <div v-if="loading" class="flex-1 flex items-center justify-center">
      <div class="flex flex-col items-center gap-3 text-muted-foreground">
        ...
        <p class="text-sm">Loading asset descriptions...</p>
      </div>
    </div>
        ...
        <TableBody>
          ...
          >
            <TableCell v-for="col in columns" :key="col.key">
              ...
              <!-- Subcategories -->
              <template v-else-if="col.key === 'A66_label'">
                <div v-if="row.A66_label" class="flex flex-wrap gap-1">
                  <Badge
                    v-for="(cat, ci) in row.A66_label.split(',').slice(0, 3)"
                    :key="ci"
                    variant="outline"
                  ...
                  <span
                    v-if="row.A66_label.split(',').length > 3"
                    class="text-[10px] text-muted-foreground"
                  >
                    +{{ row.A66_label.split(',').length - 3 }}
                  </span>
                </div>
              ...
              <!-- Default -->
              <template v-else>
                <span class="text-sm text-muted-foreground">{{ row[col.key] || '—' }}</span>
              </template>
            </TableCell>
          </TableRow>
          ...
          <TableRow v-if="visibleRows.length === 0 && !loading">
            <TableCell :colspan="columns.length" class="h-32 text-center">
              <div class="flex flex-col items-center gap-2 text-muted-foreground">
                ...
                <p>No asset descriptions found</p>
              </div>
            </TableCell>
          </TableRow>
