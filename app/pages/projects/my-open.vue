...
setHeader({ title: 'My Open Projects', icon: 'i-lucide-folder-open' })
...
// ─── Open statuses ───────────────────────────────────────────
const OPEN_STATUSES = ['in progress', 'open', 'active', 'new job', 'new']

function isOpenProject(p: any) {
  const jobStatus = String(p['Job Status'] || '').toLowerCase()
  const projectStatus = String(p['Project Status'] || '').toLowerCase()
  return OPEN_STATUSES.some(s => jobStatus.includes(s) || projectStatus.includes(s))
}
...
const filteredProjects = computed(() => {
  let list = projects.value.filter((p: any) => isOpenProject(p) && isMyProject(p))
  if (search.value) {
